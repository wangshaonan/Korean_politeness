...
for line in open(sys.argv[1]):
    ...
    if not len(text_sentence):
        out.write('\n')
        continue
    input_ids = encode(albert_tokenizer, text_sentence)
    print(input_ids)
    print(albert_tokenizer.convert_ids_to_tokens(input_ids[0]))
    out.write('Prob_Sent '+' '.join(albert_tokenizer.convert_ids_to_tokens(input_ids[0]))+'\n')
    with torch.no_grad():
        logits = albert_model(input_ids)[0]
