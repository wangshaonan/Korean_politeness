...
for line in open(sys.argv[1]):
    ...
    if not len(text_sentence):
        out.write('\n')
        continue
    input_ids = encode(xlmroberta_tokenizer, text_sentence)
    print(input_ids)
    print(xlmroberta_tokenizer.convert_ids_to_tokens(input_ids[0]))
    out.write('Prob_Sent '+' '.join(xlmroberta_tokenizer.convert_ids_to_tokens(input_ids[0]))+'\n')
    with torch.no_grad():
        logits = xlmroberta_model(input_ids)[0]
